    ...
    <div class="clearfix padding_10">
        <div class="wrap_conte_filtros">
            <div class="clearfix conte_filtros">
                  <label class="label_selects label-filter-1" for="ambito">Ámbito</label>
                  <select class="selects selects_filtros from-select" id="ambito" name="ambito" ng-model="ambito" ng-change="getAmbito()" >
                      <option value=''>Seleccione</option>
                      <!--<option value="{{Config::get('constants.C_TODOS')}}">TODOS</option>-->
                  ...
                  <input type="button" class="botones" value="EXPORTAR"  ng-show="showexport" id="btnexport" ng-disabled='disableExportar' ng-click='exportData()' ng-cloak>
...
<div style="width: 100%; height: 600px; overflow-y: scroll;" ng-show='showreporte' ng-cloak>
    ...
    <table  border="1" bordercolor="#c5dbec" width="100%" style="font-size:13px" ng-show='showreporteTable'  ng-cloak>
        ...
        <tr ng-repeat="item in lista" style="background-color:<% (item.departamento != null?'#eafadc':(item.provincia != null?'#ffffe0':''))%>"  >
            ...
            <td style="color:<% item.capital == '01'?'':(item.est_a != 0 && item.est_b == 0)?'#002CFC':'#FF0004' %>" ><% (ambito == '03' && item.departamento != null )?'':item.estado %> </td> 
        </tr>
    </table>
